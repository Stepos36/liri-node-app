...
var runProgram = function(command, request) {
    ...
    if (command==="concert-this") {
        axios.get("https://rest.bandsintown.com/artists/" + request + "/events?app_id=codingbootcamp").then(
            function(response) {
                    if((response.data.length===18)||(response.data.length===0)){
    ...
    if (command=="spotify-this-song") {
        if(request!==''){}
        else{request='The Sign Ace of Base'
    ...
    if (command==="movie-this") {
        ...
        axios.get('http://www.omdbapi.com/?t=+'+request+'&apikey=trilogy').then( function(response, error) {
            if (error) {
